...
async function paymenterConfig(env) {
  const saved = (await getDoc(env, "private_config/paymenter")) || {};
  return {
    panelUrl: normalizePanelUrl(saved.panelUrl || env.PAYMENTER_PANEL_URL || ""),
    apiKey: String(saved.apiKey || env.PAYMENTER_API_KEY || ""),
  };
}

function apiAttrs(resource) {
  return resource?.attributes || resource?.data?.attributes || resource || {};
}

async function paymenterFetch(env, path, init = {}) {
  const cfg = await paymenterConfig(env);
  if (!cfg.panelUrl || !cfg.apiKey) throw new Error("Paymenter URL or API key is not configured");
  const endpoint = `${cfg.panelUrl.replace(/\/api$/, "")}/api${path}`;
  let text = "";
  try {
    ...
    if (text) {
      try {
        ...
        throw new Error(`Paymenter non-JSON response (${res.status} ${endpoint}): ${text.slice(0, 300)}`);
      }
    }
    if (!res.ok) {
      const message = pteroApiErrorText(data) || data?.message || data?.error || "Paymenter API failed";
      throw new Error(`${message} (${res.status} ${endpoint}) ${text.slice(0, 300)}`);
    }
    return data;
  } catch (error) {
    if (error instanceof Error) throw error;
    throw new Error(`Paymenter request failed (${endpoint}): ${String(error)}`);
  }
}
...
async function createPaymenterProvisioning(env, order, plan) {
  const productId = Number(plan.paymenterProductId || 0);
  if (!productId) throw new Error("Paymenter Product ID missing on plan");
  const suffix = randomId(12);
  const credentials = {
    email: `server-${suffix}@infinityhost.online`,
    password: randomPassword(),
  ...
  const userRes = await paymenterFetch(env, "/v1/admin/users", {
    method: "POST",
    body: JSON.stringify({
      first_name: credentials.firstName,
      last_name: credentials.lastName,
  ...
  const paymenterUser = apiAttrs(userRes);
  const userId = Number(paymenterUser.id);
  if (!userId) throw new Error("Paymenter user created but user id missing in response");

  let planId = Number(plan.paymenterPlanId || 0);
  if (!planId) {
    const product = await paymenterFetch(env, `/v1/admin/products/${productId}?include=plans.prices`);
    const relPlan = product?.data?.relationships?.plans?.data?.[0];
    const includedPlan = (product?.included || []).find((item) => item.type === "plans");
    planId = Number(relPlan?.id || includedPlan?.id || includedPlan?.attributes?.id || 0);
  }
  if (!planId) throw new Error("Paymenter Plan ID missing. Fill Paymenter Plan ID in Admin > Pricing for this product.");

  const currency = String(order.currency || "INR").toUpperCase();
  const price = Number(order.amountPaid || order.amount || 0);
  const orderRes = await paymenterFetch(env, "/v1/admin/orders", {
    method: "POST",
    body: JSON.stringify({ user_id: userId, currency_code: currency }),
  });
  const paymenterOrder = apiAttrs(orderRes);
  const orderId = Number(paymenterOrder.id);

  const serviceRes = await paymenterFetch(env, "/v1/admin/services", {
    method: "POST",
    body: JSON.stringify({
  ...
  const service = apiAttrs(serviceRes);
  const cfg = await paymenterConfig(env);
  return {
    credentials,
    ...
    userId,
    orderId,
    serviceId: service.id || serviceRes?.data?.id || "",
  };
}
...
async function provisionOrder(env, order, source = "webhook") {
  ...
  try {
    ...
    if (plan.paymenterProductId) {
      const result = await createPaymenterProvisioning(env, order, plan);
      const serverConfiguration = [
        `Billing Panel URL: ${result.panelUrl}`,
        `Billing Panel Email: ${result.credentials.email}`,
        `Billing Panel Username: ${result.credentials.username}`,
        `Billing Panel Password: ${result.credentials.password}`,
        `Paymenter User ID: ${result.userId}`,
        `Paymenter Order ID: ${result.orderId}`,
        `Paymenter Service ID: ${result.serviceId}`,
        `Paymenter Product ID: ${plan.paymenterProductId}`,
        `Paymenter Plan ID: ${plan.paymenterPlanId || "auto"}`,
      ].filter(Boolean).join("\n");
      await patchDoc(env, `orders/${order.id}`, {
        provisioningStatus: "Active",
        serverProvisioned: true,
        provisioningLog: "Paymenter service created successfully",
        provisioningCompletedAt: now(),
        serverEmail: result.credentials.email,
        serverPassword: result.credentials.password,
        pteroUsername: result.credentials.username,
        panelUrl: result.panelUrl,
        paymenterUserId: String(result.userId || ""),
        paymenterOrderId: String(result.orderId || ""),
        paymenterServiceId: String(result.serviceId || ""),
        serverConfiguration,
      });
      await writeProvisionLog(env, order, "active", "Paymenter service created successfully", { paymenterServiceId: String(result.serviceId || "") });
      return { success: true, provider: "paymenter", serviceId: result.serviceId };
    }
    throw new Error("Paymenter Product ID missing on this website plan. Admin > Pricing > Edit Plan me Paymenter Product ID fill karke Save Changes karo, phir Retry dabao.");
  } catch (error) {
...
async function handlePaymenterConfig(request, env) {
  const admin = await requireAdmin(env, request);
  if (!admin) return json({ error: "Unauthorized" }, 401);
  if (request.method === "GET") {
    const cfg = await paymenterConfig(env);
  ...
  const panelUrl = normalizePanelUrl(body.panelUrl || "");
  const apiKey = String(body.apiKey || "").trim();
  if (!panelUrl) return json({ error: "Paymenter URL is required" }, 400);
  const existing = await paymenterConfig(env);
  const next = {
    panelUrl,
    apiKey: apiKey || existing.apiKey || "",
    updatedAt: now(),
  ...
  await patchOrCreateDoc(env, "private_config/paymenter", next);
  return json({ panelUrl, hasApiKey: !!next.apiKey });
}

async function handlePaymenterTest(request, env) {
  const admin = await requireAdmin(env, request);
  if (!admin) return json({ error: "Unauthorized" }, 401);
  try {
    const data = await paymenterFetch(env, "/v1/admin/products?per_page=1");
    return json({ success: true, message: "Paymenter connected", products: data?.meta?.total || data?.data?.length || 0 });
  } catch (error) {
    await createDoc(env, "paymenter_logs", {
    ...
    return json({ success: false, error: error instanceof Error ? error.message : "Paymenter test failed" }, 502);
  }
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/admin/paymenter/config" && ["GET", "PUT"].includes(request.method)) {
        const missing = missingEnv(env);
        if (missing.length) return json({ error: `Missing env vars: ${missing.join(", ")}` }, 500);
        return handlePaymenterConfig(request, env);
      }
      if (url.pathname === "/api/admin/paymenter/test" && request.method === "POST") {
        const missing = missingEnv(env);
        if (missing.length) return json({ error: `Missing env vars: ${missing.join(", ")}` }, 500);
        return handlePaymenterTest(request, env);
      }
