...
function planLimits(plan) {
  const ramMb = Math.max(128, Math.round(parseAmount(plan.ram, 1) * 1024));
  const diskMb = Math.max(512, Math.round(parseAmount(plan.ssd, 5) * 1024));
  const cpu = Math.max(10, Math.round(parseAmount(plan.players, 100)));
  return { ramMb, diskMb, cpu };
}
...
async function pteroFetch(env, path, init = {}, config = null) {
  ...
  if (!detail.ok || !data) {
    const message = data?.errors?.[0]?.detail || data?.message || detail.hint || "Pterodactyl API failed";
...
async function createPteroServer(env, order, plan, pteroUser) {
  ...
  const result = await pteroFetch(env, "/api/application/servers", {
    ...
    body: JSON.stringify({
      ...
      feature_limits: {
        databases: Number(env.PTERO_DEFAULT_DATABASES || 1),
        allocations: Number(env.PTERO_DEFAULT_ALLOCATIONS || 1),
        backups: Number(env.PTERO_DEFAULT_BACKUPS || 1),
      },
...
function validateProvisioningPlan(plan) {
  ...
  if (missing.length) {
    throw new Error(`Plan missing provisioning fields: ${missing.join(", ")}. Edit this plan in Admin > Pricing and save Auto Provisioning Mapping.`);
  }
}
