...
function validateProvisioningPlan(plan) {
  ...
  if (!Number(plan?.pteroMemoryMb || 0)) missing.push("Pterodactyl Memory MB");
  if (!Number(plan?.pteroDiskMb || 0)) missing.push("Pterodactyl Disk MB");
  if (!Number(plan?.pteroCpuPercent || 0)) missing.push("Pterodactyl CPU %");
  if (missing.length) {
